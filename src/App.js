...
export default function App() {
  ...
  function getResult() {
    ...
    if (randomNumber < weapons[selectedWeapon][selectedBodyPart]) {
      setResult("Hit!");
      setAnimationClass("animate-hit");
    } else if (randomNumber > weapons[selectedWeapon][selectedBodyPart]) {
      setResult("Miss!");
      setAnimationClass("animate-miss");
    } else {
      setResult("BAN");
      setAnimationClass("animate-ban");
    }
  }
  ...
  return (
    <main className="bg-image-container">
      <div className="container mx-auto">
        ...
        <p
          className={`text-slate-100 text-center mt-5 text-xl ${animationClass}`}
        >
          {result}
        </p>
      </div>
